...
@export var LimbController : LimbPlacementController
@export var MinimumAttackDelay : float = 1.5
@export var MaximumAttackDelay : float = 4.0
@export var AttackInitiateDistance : float = 6.0
@export var AttackDamageDistance : float = 2.0
@export var AttackForce : float = 30.0
...
func ResetAttackCooldown():
	...
	rng.randomize();
	AttackCooldown = rng.randf_range(MinimumAttackDelay, MaximumAttackDelay);
	IsAttacking = false;
...
func OnDamaged(hitLocation : Vector3, force : Vector3, AggressorBodyNode : Node3D):
	...
	if (CurrentHealth <= 0):
		if (DeathSounds.size() > 0):
			get_tree().call_group("AudioQues", "PlayAudioQue3D", DeathSounds[RandomNumberGenerator.new().randi_range(0, DeathSounds.size() - 1)], BaseSkeleton.global_position, 0)
		OnDeath(hitLocation, force)
	else:
		if (DamageSounds.size() > 0):
			get_tree().call_group("AudioQues", "PlayAudioQue3D", DamageSounds[RandomNumberGenerator.new().randi_range(0, DamageSounds.size() - 1)], BaseSkeleton.global_position, 0)
...
func AttackTarget(parentNode : Node3D) -> bool:
	var damageable : DamageableObject = parentNode.get_node("Damageable") as DamageableObject
	if (damageable != null):
		damageable.HitObject(NavigationAgent.global_position, (parentNode.global_position - NavigationAgent.global_position).normalized() * AttackForce, NavigationAgent)
		
		var bounceVector : Vector3 = (NavigationAgent.global_position - parentNode.global_position).normalized() * AttackForce
		SetPathingTarget(NavigationAgent.global_position + bounceVector);
		NavigationAgent.apply_impulse(bounceVector);
		
		return true;
	
	return false

func CheckTargetValid():
...
func _process(delta):
	...
	if (IsAttacking):
		...
		if (CurrentTarget.global_position.distance_to(NavigationAgent.global_position) <= AttackDamageDistance):
			ResetAttackCooldown();
	...
	# If that is not present use the current target node if it is present, simply chasing.
		...
		if (AttackCooldown == 0 && CurrentTarget.global_position.distance_to(NavigationAgent.global_position) < AttackInitiateDistance && !LimbController.LaunchingRequested):
			LimbController.OnAttackLaunchRequested(CurrentTarget);
			NavigationAgent.SetNewTarget(CurrentTarget.global_position);
			IsAttacking = true;
			AttackCooldown = (CurrentTarget.global_position.distance_to(NavigationAgent.global_position) / (NavigationAgent.MaximumVelocity * LimbController.LaunchVelocityMultiplier)) * 2.0;
