...
@export var DissolveSounds : Array[AudioStream] = []
...
func _process(delta):
	if (fadeDelay > 0):
		...
		if (!spawnedParticles):
			if (DissolveSounds.size() > 0):
				get_tree().call_group("AudioQues", "PlayAudioQue3D", DissolveSounds[RandomNumberGenerator.new().randi_range(0, DissolveSounds.size() - 1)], DissolveSpawnPoints[0].global_position, 0)
