...
@export var GunshotSounds : Array[AudioStream] = []
@export var PlayerBodyNode: Node3D;
...
func FireRevolver():
	...
	if (GunshotSounds.size() > 0):
		get_tree().call_group("AudioQues", "PlayAudioQue3D", GunshotSounds[RandomNumberGenerator.new().randi_range(0, GunshotSounds.size() - 1)], BarrelEnd.global_position, 5)
	...
	if (BarrelRayCast.is_colliding()):
		...
		if (impactedTarget.has_node("Damageable")):
			...
			damageable.HitObject(hitLocation, -BarrelRayCast.global_transform.basis.z * ImpactForce, PlayerBodyNode);
