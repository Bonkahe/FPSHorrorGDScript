...
func OnDamaged(hitLocation : Vector3, force : Vector3, AggressorBodyNode : Node3D):
	if (HurtSounds.size() > 0):
		get_tree().call_group("AudioQues", "PlayAudioQue", HurtSounds[RandomNumberGenerator.new().randi_range(0, HurtSounds.size() - 1)], 0)
	...
	#Handle death later.
	if (CurrentHealth == 0):
		get_tree().call_group("AudioQues", "PlayAudioQue", PlayerDeathSound, 0)
