...
func _physics_process(delta):
	...
	if not is_on_floor():
		...
		if (lastYVelocity < -CameraShake_FallingBias):
			...
			if (FootstepSounds.size() > 0):
				get_tree().call_group("AudioQues", "PlayAudioQue3D", FootstepSounds[RandomNumberGenerator.new().randi_range(0, FootstepSounds.size() - 1)], CameraNode.global_position + (Vector3.DOWN * 2.0), 0)
	...
	if (walkingSway_CurrentValue > 0):
		...
		if(stepBounce == WalkingSway_MaxSwayHandsHeight):
			if (FootstepSounds.size() > 0):
				get_tree().call_group("AudioQues", "PlayAudioQue3D", FootstepSounds[RandomNumberGenerator.new().randi_range(0, FootstepSounds.size() - 1)], CameraNode.global_position + (Vector3.DOWN * 2.0), -2)
	...
	if Input.is_action_just_pressed("Jump") and is_on_floor():
		...
		if (FootstepSounds.size() > 0):
			get_tree().call_group("AudioQues", "PlayAudioQue3D", FootstepSounds[RandomNumberGenerator.new().randi_range(0, FootstepSounds.size() - 1)], CameraNode.global_position + (Vector3.DOWN * 2.0), 3)
