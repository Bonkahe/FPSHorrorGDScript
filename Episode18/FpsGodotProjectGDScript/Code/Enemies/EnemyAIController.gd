...
func AttackTarget(parentNode : Node3D) -> bool:
	...
	if (damageable != null):
		damageable.HitObject(NavigationAgent.global_position, (parentNode.global_position - NavigationAgent.global_position).normalized() * AttackForce, self)
		var bounceVector : Vector3 = (NavigationAgent.global_position - parentNode.global_position).normalized() * AttackForce * 2.0;
