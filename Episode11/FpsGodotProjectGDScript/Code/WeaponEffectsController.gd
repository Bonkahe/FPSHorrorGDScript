...
var currentTiltWeight: float;
var lastTiltOffsetVector: Vector3;
var lastTiltLookAtVector: Vector3;
...
func _physics_process(delta):
	...
	if (IsAiming):
		...
		AimingIKContainer.global_transform = AdjustToEnvironment(AimingIKContainer.global_transform, delta);
	else:
		...
		RightHandIdleIKContainer.global_transform = AdjustToEnvironment(RightHandIdleIKContainer.global_transform, delta);

func AdjustToEnvironment(targetGlobalTransform : Transform3D, deltaTime : float) -> Transform3D:
